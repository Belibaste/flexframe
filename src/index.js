 ...
 static get toolbox() {
  ...
  set data(data) {
    if (!(data instanceof Object)) {
      throw Error('Flexframe Tool data should be object');
    }
  ...
  get CSS() {
    return {
      ...
      container: 'flexframe-tool',
      containerLoading: 'flexframe-tool--loading',
      preloader: 'flexframe-tool__preloader',
      caption: 'flexframe-tool__caption',
      url: 'flexframe-tool__url',
      content: 'flexframe-tool__content',
    };
  }
  ...
  onPaste(event) {
    ...
    const { regex, embedUrl, width, height, id = (ids) => ids.shift() } = Flexframe.services[service];
  ...
  static prepare({ config = {} }) {
    ...
    const userServices = Object
      ...
      .filter(([key, service]) => Flexframe.checkServiceConfig(service))
      .map(([key, service]) => {
    ...
    Flexframe.services = entries.reduce((result, [key, service]) => {
      if (!(key in result)) {
        result[key] = service;
